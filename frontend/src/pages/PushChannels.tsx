...
export default function PushChannels() {
  ...
  const load = async (): Promise<boolean> => {
    ...
    setLoading(true)
    try {
      const channelsRes = await api.get<ResultEnvelope<Channel[]>>('/channels')
  ...
  return (
    <div className="channels-page">
      ...
      <div className="page-header">
        <h2>🔔 推送渠道</h2>
        <p className="page-desc">这里只保存账号。每个主题要推到哪里，去订阅管理里绑定；不绑定就只在网页看。</p>
      </div>
      ...
      {!loading && (
        <div className="delivery-settings">
          <div className="delivery-header">
            <div>
              <h3>绑定位置</h3>
              <p>通讯录只负责存账号。主题、时间和推送绑定都在订阅管理里完成。</p>
            </div>
            <Link to="/subscription" className="btn-primary">去订阅管理 →</Link>
          </div>
        </div>
      )}
